...
export default function App() {
  ...
  const [sessionToken, setSessionToken] = useState<string | null>(() => localStorage.getItem('aurafit_token'));
  ...
  const [isOnboarded, setIsOnboarded] = useState<boolean>(() => {
    return localStorage.getItem('aurafit_onboarded') === 'true';
  });
  ...
  const [dailyBurned, setDailyBurned] = useState<number>(() => {
    const saved = localStorage.getItem('aurafit_daily_burned');
    return saved ? parseInt(saved) : 0;
  });
  const dailyBurnedTarget = 550;
  const [dailyWater, setDailyWater] = useState<number>(() => {
    const saved = localStorage.getItem('aurafit_daily_water');
    return saved ? parseInt(saved) : 0;
  });
  
  const [dailyWaterTarget, setDailyWaterTarget] = useState<number>(() => {
    const saved = localStorage.getItem('aurafit_water_target');
    return saved ? parseInt(saved) : 2500;
  });

  const [isProfileDrawerOpen, setIsProfileDrawerOpen] = useState(false);

  // Custom logging items state (Breakfast, Lunch, Dinner, Snack checks)
  ...
  // Athlete Profile Editor drafts state
  ...
  // Daily advice state (Tip of the Day)
  const [dailyTip, setDailyTip] = useState<string>('Generating customized wellness insights...');
  const [isTipLoading, setIsTipLoading] = useState<boolean>(false);

  // Telemetry logs
  const [telemetry, setTelemetry] = useState<TelemetryLog[]>(() => {
    const saved = localStorage.getItem('aurafit_telemetry');
    return saved ? JSON.parse(saved) : [];
  });
  ...
  useEffect(() => {
    const loadSession = async () => {
      ...
      if (!sessionToken || authStep === 'otp') return;
      try {
  ...
  // Persists Meal history
  useEffect(() => {
    localStorage.setItem('aurafit_meals', JSON.stringify(mealLogs));
  }, [mealLogs]);

  // Persists Weight patterns
  useEffect(() => {
  ...
  // Persists logs
  useEffect(() => {
    localStorage.setItem('aurafit_telemetry', JSON.stringify(telemetry));
  }, [telemetry]);
  ...
  // Persists chatroom messages
  useEffect(() => {
    localStorage.setItem('aurafit_messages', JSON.stringify(messages));
  }, [messages]);

  // Load appropriate workout set based on onboarding parameters
  useEffect(() => {
    const key = `${profile.location}-${profile.experience}`;
    if (PRESET_WORKOUTS[key]) {
      // Create a fresh clone so logging completed exercises doesn't mess up presets
      const cloned = JSON.parse(JSON.stringify(PRESET_WORKOUTS[key]));
      setActiveWorkout(cloned);
      if (cloned.exercises.length > 0) {
  ...
  const handleQuickLogMeal = (mealType: 'breakfast' | 'lunch' | 'dinner' | 'snack') => {
    const prefKey = profile.dietaryPref;

    let mealTemplate = { name: "Snack Item", calories: 150, carbs: 12, protein: 10, fat: 4 };

    if (mealType === 'breakfast') {
      mealTemplate = BREAKFAST_RECIPES[prefKey] || BREAKFAST_RECIPES['Standard'];
    } else if (mealType === 'lunch') {
      mealTemplate = LUNCH_RECIPES[prefKey] || LUNCH_RECIPES['Standard'];
    } else if (mealType === 'dinner') {
      mealTemplate = DINNER_RECIPES[prefKey] || DINNER_RECIPES['Standard'];
    } else {
      // Snack fallback helper
      mealTemplate = { name: `${prefKey} High-Energy Crunch Bar`, calories: 210, carbs: 15, protein: 14, fat: 8 };
    }
  ...
  const handleResetApplicationState = () => {
    ...
    setDailyWater(1250);
    setMealLogs([
      { id: 'm_1', type: 'breakfast', name: 'Almond Chia Protein Shake', calories: 350, carbs: 20, protein: 35, fat: 12 }
    ]);
                  ...
                  <div className="bg-[#1C1C1E] border border-gray-800 rounded-2xl p-5 space-y-3 shadow-[0_4px_20px_rgba(255,255,255,0.01)]">
                    ...
                    <div className="flex items-center justify-between p-3.5 bg-zinc-950/40 border border-zinc-900 rounded-xl">
                      <div>
                        ...
                        <h4 className="text-xs font-semibold text-white mt-0.5">
                          {BREAKFAST_RECIPES[profile.dietaryPref]?.name || BREAKFAST_RECIPES['Standard'].name}
                        </h4>
                        <p className="text-[10px] text-zinc-500 font-mono mt-1">
                          {BREAKFAST_RECIPES[profile.dietaryPref]?.calories || BREAKFAST_RECIPES['Standard'].calories} kcal |{' '}
                          {BREAKFAST_RECIPES[profile.dietaryPref]?.protein || BREAKFAST_RECIPES['Standard'].protein}g Pro
                        </p>
                      </div>
                    ...
                    <div className="flex items-center justify-between p-3.5 bg-zinc-950/40 border border-zinc-900 rounded-xl">
                      <div>
                        ...
                        <h4 className="text-xs font-semibold text-white mt-0.5">
                          {LUNCH_RECIPES[profile.dietaryPref]?.name || LUNCH_RECIPES['Standard'].name}
                        </h4>
                        <p className="text-[10px] text-zinc-500 font-mono mt-1">
                          {LUNCH_RECIPES[profile.dietaryPref]?.calories || LUNCH_RECIPES['Standard'].calories} kcal |{' '}
                          {LUNCH_RECIPES[profile.dietaryPref]?.protein || LUNCH_RECIPES['Standard'].protein}g Pro
                        </p>
                      </div>
                    ...
                    <div className="flex items-center justify-between p-3.5 bg-zinc-950/40 border border-[#8B5CF6]/20 rounded-xl">
                      <div>
                        ...
                        <h4 className="text-xs font-semibold text-white mt-0.5">
                          {DINNER_RECIPES[profile.dietaryPref]?.name || DINNER_RECIPES['Standard'].name}
                        </h4>
                        <p className="text-[10px] text-zinc-500 font-mono mt-1">
                          {DINNER_RECIPES[profile.dietaryPref]?.calories || DINNER_RECIPES['Standard'].calories} kcal |{' '}
                          {DINNER_RECIPES[profile.dietaryPref]?.protein || DINNER_RECIPES['Standard'].protein}g Pro
                        </p>
                      </div>
              ...
              <div className="bg-[#1C1C1E] border border-gray-800 rounded-2xl flex flex-col h-[525px] relative overflow-hidden shadow-[0_4px_25px_rgba(139,92,246,0.03)]">
                ...
                <div className="p-4 border-b border-zinc-900 bg-zinc-950/45 flex items-center justify-between">
                  ...
                  <button
                    onClick={() => {
                      setMessages([
                        { id: 'init_1', sender: 'aura', text: "Chat cleared. Ask me any structured physiological query." }
                      ]);
